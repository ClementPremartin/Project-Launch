...
const addWilder = async (req: Request, res: Response) => {
    const {firstname, lastname, description, schoolId} = req.body;
    if(!firstname || !lastname) {
        res.status(400).json({error: "Firstname and Lastname are required."})
    } else {
        const newWilder = await WilderRepository.createWilder(firstname, lastname, description, schoolId);
...
const modifyWilderById = async (req: Request, res: Response) => {
    ...
    const {firstname, lastname, description} = req.body;
        const modifyWilder = await WilderRepository.putWilder(id, firstname, lastname, description);
...
const addSkills = async (req: Request, res: Response) => {
    ...
    if(!skillId) {
        ...
        try {
            const updatedWilder = await WilderRepository.addSkillsToWilder(wilderId, skillId);
